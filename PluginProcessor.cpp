...
#include <fstream>  // Read a txt file
#include <vector>
#include <sstream>
...
std::vector<float> loadFIRCoefficients(const juce::String& filePath)
{
    std::vector<float> coefficients;
    std::ifstream file(filePath.toStdString());  // Open the file in lecture mode
...
void PFE_WFS_simpleAudioProcessor::prepareToPlay(double sampleRate, int samplesPerBlock)
{
    ...
    // Get the path of the coeff file
    juce::File coefficientsFile = juce::File::getSpecialLocation(juce::File::currentApplicationFile)
        .getSiblingFile("FIR_b_coeffs.txt");

    // Load the coefficients
    std::vector<float> firCoeffs = loadFIRCoefficients(coefficientsFile.getFullPathName());

    // Initialize the fir filter
    firFilter.coefficients = juce::dsp::FIR::Coefficients<float>::Ptr(
...
void PFE_WFS_simpleAudioProcessor::processBlock(juce::AudioBuffer<float>& buffer, juce::MidiBuffer& midiMessages)
{
   ...
   //Creation d'un tableau de gains + gain
    ...
    for (int channel = 0; channel < totalNumOutputChannels; channel++) {
        ...
        //gain (channel-i�me float du tableau gainArray)
        gain = gainArray[channel];

        // loop on each samples for a given channel
        for (int sample = 0; sample < buffer.getNumSamples(); sample++) {
            ...
            if (params._bypassGain == 1.0) {
                ...
                float filteredSample = firFilter.processSample(inSamplef);
                output[sample] = filteredSample;
            }
                ...
                // Apply the FUR filter to each sample
                float filteredSample = firFilter.processSample(inSamplef);
                output[sample] = filteredSample;
            }
